...
import { trigger, state, style, animate, transition } from '../../../node_modules/@angular/core';
...
@Component({
  ...
  animations:[
    trigger('openingAnimation', [
      ...
      transition('hidden => shown', animate('.6s'))
    ])
  ]
})
export class AlarmComponent implements OnInit,AfterViewInit {
  ...
  deleteAlarm(alarm: AlarmModel) {
    this.alarmIndex = this.alarmModel.indexOf(alarm);
    this.deleteModal.show();

  }

  generateAlarmId() {
    return Math.max.apply(Math, this.alarmModel.map(function (eachAlarm) { return eachAlarm.id; }))
  }
  deletethisAlarm() {
    this.alarmModel.splice(this.alarmIndex, 1);
    localStorage.setItem("alarm", JSON.stringify(this.alarmModel));
